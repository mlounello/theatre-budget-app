...
\echo 'Phase 4 Gate 2: purchase trigger rollback test'
...
begin
  ...
  insert into purchases (project_id, title, budget_tracked)
  values (v_project_id, '[PHASE4 TEST - ROLLBACK] legacy writer', false)
  returning id, fiscal_year_id into v_purchase_id, v_assigned_fiscal_year_id;

  if v_assigned_fiscal_year_id is distinct from v_project_fiscal_year_id then
    raise exception 'Legacy purchase writer did not receive project fiscal year.';
  end if;

  insert into purchases (project_id, fiscal_year_id, title, budget_tracked)
  values (
    v_project_id,
    v_other_fiscal_year_id,
    '[PHASE4 TEST - ROLLBACK] conflicting writer',
    false
  )
  returning id, fiscal_year_id into v_purchase_id, v_assigned_fiscal_year_id;

  if v_assigned_fiscal_year_id is not null then
    raise exception 'Conflicting fiscal year was accepted.';
  end if;

  if not exists (
    select 1 from fiscal_year_assignment_conflicts conflict
    where conflict.entity_table = 'purchases'
      and conflict.entity_id = v_purchase_id
      and conflict.conflict_type = 'explicit_fiscal_year_mismatch'
      and conflict.status = 'open'
  ) then
    raise exception 'Conflicting purchase did not create an open conflict.';
  end if;
end;
$$;
