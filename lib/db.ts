...
export type GuestArtistOption = {
  id: string;
  displayName: string;
  vendorNumber: string | null;
  email: string | null;
  phone: string | null;
  defaultFoapalId: string | null;
  defaultCheckRequestHandling: "mail" | "business_affairs_pickup" | "other";
  defaultCheckRequestOtherLocation: string | null;
  vendorAddress1: string | null;
  vendorAddress2: string | null;
  vendorAddress3: string | null;
  taxIdLast4: string | null;
...
export type ProjectDeletionImpact = {
  ...
  budgetLines: number;
  purchases: number;
  creditCardStatementMonths: number;
  incomeEntries: number;
  contracts: number;
  projectMemberships: number;
  productionTeamAssignments: number;
  accessScopesDetached: number;
  associatedContractsDetached: number;
};
...
export type IncomeRow = {
  ...
  explicitFiscalYearId: string | null;
  fiscalYearId: string | null;
  fiscalYearName: string | null;
  projectId: string | null;
...
export async function getDashboardOrganizationBudgets(
  ...
  const supabase = await getSupabaseServerClient();
  const allOrganizations = await getFiscalYearOrganizationOptions(params.fiscalYearId);
  const allOrganizationIds = [...new Set(allOrganizations.map((row) => row.id))];
  if (allOrganizationIds.length === 0) return [];

  let directPurchasesQuery = supabase
    .from("purchases")
    .select("fiscal_year_id, organization_id, status, estimated_amount, requested_amount, encumbered_amount, pending_cc_amount, posted_amount")
    .is("project_id", null)
    .in("organization_id", allOrganizationIds);
  if (params.fiscalYearId) directPurchasesQuery = directPurchasesQuery.eq("fiscal_year_id", params.fiscalYearId);
  const { data: directPurchases, error: directPurchasesError } = await directPurchasesQuery;
  if (directPurchasesError) throw directPurchasesError;

  const organizationsWithDirectPurchases = new Set(
    (directPurchases ?? []).map((purchase) => `${purchase.fiscal_year_id as string}:${purchase.organization_id as string}`)
  );
  const organizations = allOrganizations.filter(
    (row) => !row.projectTrackingRequired || organizationsWithDirectPurchases.has(`${row.fiscalYearId}:${row.id}`)
  );
  ...
  let plansQuery = supabase
    .from("budget_plans")
    .select("fiscal_year_id, organization_id, annual_amount, account_codes(is_revenue)")
    .in("organization_id", organizationIds);
  if (params.fiscalYearId) plansQuery = plansQuery.eq("fiscal_year_id", params.fiscalYearId);
  ...
  for (const plan of plans ?? []) {
    ...
    const organizationId = `${plan.fiscal_year_id as string}:${plan.organization_id as string}`;
  ...
  for (const purchase of purchases ?? []) {
    const organizationId = `${purchase.fiscal_year_id as string}:${purchase.organization_id as string}`;
  ...
  return organizations.map((row) => {
    ...
    const scopeKey = `${row.fiscalYearId}:${organizationId}`;
    const allocatedTotal = allocatedByOrganization.get(scopeKey) ?? 0;
    const totals = totalsByOrganization.get(scopeKey) ?? { requested: 0, held: 0, enc: 0, pending: 0, ytd: 0 };
...
export async function getDashboardOpenRequisitions(
  ...
  if (budgetType !== "non_theatre") {
    let query = supabase
      .from("purchases")
      .select(
        "id, fiscal_year_id, project_id, title, requisition_number, po_number, procurement_status, estimated_amount, requested_amount, encumbered_amount, posted_amount, projects!inner(name, season), vendors(name)"
      )
  ...
  if (budgetType !== "theatre") {
    let query = supabase
      .from("purchases")
      .select(
        "id, fiscal_year_id, organization_id, title, requisition_number, po_number, procurement_status, estimated_amount, requested_amount, encumbered_amount, posted_amount, organizations!inner(name, org_code), vendors(name)"
      )
      .is("project_id", null)
      .eq("request_type", "requisition")
      .neq("procurement_status", "paid")
...
export async function getProcurementData(
  ...
  const supabase = await getSupabaseServerClient();
  const access = await getAccessContext();
  const pageSize = Math.min(Math.max(Math.trunc(params.pageSize ?? 50), 10), 100);
  ...
  const [
    ...
    supabase
      ...
      .not("name", "ilike", "external procurement")
      .order("name", { ascending: true }),
    getFiscalYearOrganizationOptions(params.fiscalYearId),
    supabase
      .from("account_codes")
      .select("id, code, category, name, is_revenue")
      .eq("active", true)
  ...
  const organizationOptions = organizationOptionsResponse;
...
export async function getCcPendingRows(params: { fiscalYearId?: string } = {}): Promise<
  Array<{
    scopeId: string;
    projectId: string | null;
    scopeLabel: string;
...
> {
  ...
  let query = supabase
    .from("purchases")
    .select(
      "id, project_id, organization_id, banner_account_code_id, pending_cc_amount, projects(name, season), organizations(name, org_code), account_codes(code), project_budget_lines(budget_code), credit_cards(nickname)"
    )
    .eq("status", "pending_cc")
    .gt("pending_cc_amount", 0);
  if (params.fiscalYearId) query = query.eq("fiscal_year_id", params.fiscalYearId);
  const { data, error } = await query;
  ...
  for (const row of data ?? []) {
    ...
    const scopeId = projectId ?? `organization:${organizationId ?? "unknown"}`;
    const scopeLabel = project
      ? `${project.name ?? "Project"}${project.season ? ` (${project.season})` : ""}`
      : `${organization?.org_code ?? "-"} | ${organization?.name ?? "Organization Budget"}`;
    const budgetCode = account?.code ?? budgetLine?.budget_code ?? "UNASSIGNED";
    const creditCardName = card?.nickname ?? null;
    const key = `${scopeId}:${budgetCode}:${creditCardName ?? ""}`;
    const current = grouped.get(key) ?? { scopeId, projectId, scopeLabel, budgetCode, creditCardName, pendingCcTotal: 0 };
    current.pendingCcTotal += asNumber(row.pending_cc_amount as string | number | null);
    grouped.set(key, current);
  }
  return [...grouped.values()].sort((a, b) => a.scopeLabel.localeCompare(b.scopeLabel) || a.budgetCode.localeCompare(b.budgetCode));
}
...
export async function getProjectDeletionImpact(projectId: string): Promise<ProjectDeletionImpact> {
  const supabase = await getSupabaseServerClient();

  async function countLinkedRows(
    table: string,
    column = "project_id",
    excludeDirectProject = false,
    ...
    let query = supabase
      ...
      .eq(column, projectId);
    if (excludeDirectProject) query = query.neq("project_id", projectId);
    const { count, error } = await query;
    if (error) {
      if (optionalColumn && (error.code === "42703" || error.message.includes(column))) return 0;
      throw error;
    }
    return count ?? 0;
  }
  ...
  const [
    budgetLines,
    purchases,
    creditCardStatementMonths,
    incomeEntries,
    contracts,
    projectMemberships,
    productionTeamAssignments,
    accessScopesDetached,
    associatedContractsDetached
  ] = await Promise.all([
    countLinkedRows("project_budget_lines"),
    countLinkedRows("purchases"),
    countLinkedRows("cc_statement_months"),
    countLinkedRows("income_lines"),
    countLinkedRows("contracts"),
...
export async function getFiscalYearOrganizationOptions(fiscalYearId?: string | null): Promise<OrganizationOption[]> {
  ...
  let query = supabase
    ...
    .select(
      "id, fiscal_year_id, sort_order, project_tracking_required, fiscal_years(name), organizations(id, name, org_code, active, superseded_by_organization_id)"
    )
    .eq("active", true)
    .order("sort_order", { ascending: true });
  ...
  const options = (data ?? [])
    .map((row): OrganizationOption | null => {
      const organization = row.organizations as
        | {
            id?: string;
            name?: string;
        ...
        | null;
      if (!organization?.id || organization.active === false || organization.superseded_by_organization_id) return null;
      const fy = row.fiscal_years as { name?: string } | null;
      const fiscalYearName = fy?.name ?? null;
      return {
        id: organization.id,
        ...
        sortOrder: (row.sort_order as number | null) ?? 0,
        projectTrackingRequired: (row.project_tracking_required as boolean | null) ?? true,
        label: `${organization.org_code ?? ""} | ${organization.name ?? "Organization"}${
  ...
  const onePerFiscalYearAndCode = new Map<string, OrganizationOption>();
  for (const option of options) {
    const key = `${option.fiscalYearId ?? "global"}:${option.orgCode.trim().toLowerCase()}`;
    if (!onePerFiscalYearAndCode.has(key)) onePerFiscalYearAndCode.set(key, option);
  }

  return Array.from(onePerFiscalYearAndCode.values()).sort(
      (a, b) =>
...
export async function getGuestArtistOptions(): Promise<GuestArtistOption[]> {
  const supabase = await getSupabaseServerClient();
  const { data, error } = await supabase
    .from("guest_artists")
    .select(
  ...
  if (error) {
    if (error.message.toLowerCase().includes("guest_artists")) return [];
    throw error;
  }
  return (data ?? []).map((row) => ({
    id: row.id as string,
    ...
    defaultFoapalId: (row.default_foapal_id as string | null) ?? null,
    defaultCheckRequestHandling: ((row.default_check_request_handling as string | null) ?? "mail") as
      | "mail"
      | "business_affairs_pickup"
      | "other",
    defaultCheckRequestOtherLocation: (row.default_check_request_other_location as string | null) ?? null,
    vendorAddress1: (row.vendor_address1 as string | null) ?? null,
    vendorAddress2: (row.vendor_address2 as string | null) ?? null,
    vendorAddress3: (row.vendor_address3 as string | null) ?? null,
    taxIdLast4: (row.tax_id_last4 as string | null) ?? null,
...
export async function getCategoryActualRows(): Promise<CategoryActualRow[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "project_id, fiscal_year_id, fiscal_year_name, org_code, organization_name, production_category, requested_total, held_total, enc_total, pending_cc_total, posted_total, obligated_total, projects(name)"
    )
...
export async function getBannerCodeActualRows(): Promise<BannerCodeActualRow[]> {
  ...
  const { data, error } = await supabase
    .from("v_actuals_by_banner_code")
    .select(
      "project_id, fiscal_year_id, fiscal_year_name, org_code, organization_name, banner_account_code, banner_category, banner_name, requested_total, held_total, enc_total, pending_cc_total, posted_total, obligated_total"
    )
    .order("banner_account_code", { ascending: true });
  if (error) throw error;
  ...
  for (const row of data ?? []) {
    ...
    const organizationName = (row.organization_name as string | null) ?? null;
...
export async function getIncomeRows(): Promise<IncomeRow[]> {
  ...
  const withType = await supabase
    .from("income_lines")
    .select(
      "id, fiscal_year_id, project_id, organization_id, production_category_id, banner_account_code_id, line_name, reference_number, amount, received_on, created_at, income_type, income_fiscal_year:fiscal_years!income_lines_fiscal_year_id_fkey(name), projects(name, fiscal_year_id, fiscal_years(name)), organizations(name, org_code), production_categories(name), account_codes(code)"
    )
    .order("created_at", { ascending: false })
    .limit(500);
  ...
  return (withType.data ?? []).map((row) => {
    const project = row.projects as { name?: string; fiscal_year_id?: string | null; fiscal_years?: { name?: string } | null } | null;
    const org = row.organizations as { name?: string; org_code?: string } | null;
    const productionCategory = row.production_categories as { name?: string } | null;
    const accountCode = row.account_codes as { code?: string } | null;
    const explicitFiscalYear = row.income_fiscal_year as { name?: string } | null;
    const orgLabel = org ? `${org.org_code ?? ""} | ${org.name ?? "Organization"}` : "Unassigned Organization";
    const receivedOn = (row.received_on as string | null) ?? null;
    const createdAt = row.created_at as string;
    const explicitFiscalYearId = (row.fiscal_year_id as string | null) ?? null;
    const fiscalYearId = explicitFiscalYearId;
    const fiscalYearName = explicitFiscalYear?.name ?? null;
