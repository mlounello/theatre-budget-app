-- Exact functional rollback for the Phase 3 Theatre Budget containment migration.
-- No application rows are changed or removed.
...
alter table app_theatre_budget.budget_plan_months disable row level security;
alter table app_theatre_budget.budget_plans disable row level security;

drop function if exists app_theatre_budget.can_manage_institutional_budget(uuid, uuid);
drop function if exists app_theatre_budget.can_read_institutional_budget(uuid, uuid);
